...
{   
    ...
    for(;;) {    
        // Send raw ADC light data every 5 seconds
        raw_light_val = adc1_get_raw(ADC1_CHANNEL_6);
        sprintf(payload, "Light: %d", raw_light_val);
        esp_mqtt_client_publish(client, "esp32/light", payload, strlen(payload), 0, false);
